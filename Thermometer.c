...
int main(int argc, char **argv) 
{
	...
	while (1)
	{
		...
		sprintf(ct, "Current Temperature: %d C", currTemp);
		sprintf(ht, "Highest Temperature: %d C", highestTemp);
		sprintf(lt, "Lowest Temperature: %d C", lowestTemp);
		
		
		if (abs(highestTemp - currTemp) || abs(lowestTemp - currTemp) >= 1) 
		{
			ifttt("https://maker.ifttt.com/trigger/temp/with/key/lOpF-M6u0dtlZphE9xEYCeo74fs72hpPgbSRIlmgyDe", ct, ht, lt);
		}
